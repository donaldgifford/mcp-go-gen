...
- [ ] OpenAPI-aware URL interpolation in the inline backend template (path params substituted, query params encoded) — the scaffold is in place via `BackendParam`; the template body hasn't been wired for OpenAPI-sourced params yet. _GET method covered by IMPL-0002 Phase 1 (path-param + query-param + header-param emission for any `Backend.Method == "GET"` tool, regardless of whether HCL or OpenAPI sourced). POST/PUT/PATCH/DELETE still pending generator write-mutation work._
...
- [ ] Add a `/healthz` endpoint to the generated MCP server (returns 204) so consumers like Compose can use `depends_on.service_healthy`. Surfaced by IMPL-0002 (demo harness), which works around the gap with `service_started` for now. Schema decision pending: hard-code `/healthz` into the main handler mux, or expose it via the HCL `server` block (`health_path = "/healthz"` with default).
- [ ] OIDC token forwarding through the proxy — `proxy.bearer.token_source = "request_context"` (or similar) HCL surface so the generated tool reads the validated upstream bearer from the request context (set by the OIDC auth middleware) instead of an env var. Enables end-to-end identity propagation in OAuth2 demos. Surfaced by IMPL-0002 Phase 5, which ships the simpler "separate service-account bearer" model (option b) for the initial OIDC demo. Requires: HCL schema extension, validated-token plumbing in the auth middleware contract, template change in `tools.go.tmpl` to read from context.
