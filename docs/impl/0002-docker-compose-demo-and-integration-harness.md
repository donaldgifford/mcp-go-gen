...
- [x] **Tool result shape (Resolved OQ #1):** once Phase 2 is up and the inspector is calling Phase 1's GET tools end-to-end with text-shaped results, evaluate whether `tools.go.tmpl` should switch to mark3labs/mcp-go's structured-content helpers. Score on: helper API stability, inspector rendering improvement for JSON, decode-error fallback complexity. If yes, edit `tools.go.tmpl` and the Phase 1 golden fixtures in this same IMPL. _Switched. mcp-go's `NewToolResultStructured(parsed, fallbackText)` provides both structured + text content. The renderer emits a `toolResultFromBody(body []byte)` helper at the bottom of `tools.go` (gated on `.Proxy`) that tries `json.Unmarshal` and falls back to `NewToolResultText` on parse failure — handles JSON and non-JSON upstream responses uniformly. Phase 1 golden fixture refreshed; shape test extended to pin both the helper call and its definition._
- [x] **Path-param substitution to runtime helper (Resolved OQ #2):** once Phase 2 confirms the inline `strings.NewReplacer` approach works for the demo's path-param tools, refactor to a runtime helper on `Backend` (e.g. `Backend.SubstitutePath(template string, params map[string]string) string`). Update `tools.go.tmpl` to emit calls into the helper instead of the inline literal; update `backend.go.tmpl` to define it; update Phase 1 golden fixtures. _Done. `(*Backend).SubstitutePath(template, params)` lives in `backend.go.tmpl` and centralizes `url.PathEscape` per replacement. `tools.go.tmpl` emits a `pathParams := map[string]string{...}` allocation followed by `t.Backend.SubstitutePath(...)`. Tools without path params skip the map allocation entirely (early-return in the helper plus a template branch on `$tool.Backend.PathParams`)._
